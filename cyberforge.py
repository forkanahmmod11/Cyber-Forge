...
MAX_REQUESTS = 50
MIN_DELAY = 0.20
UA = "CyberForge/6.2 (Authorized Defensive Assessment)"
...
def malware_analyze(path):
    ...
    return save_report("malware_analysis",{"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"analysis":meta})
...
def security_headers_audit(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"status":r.status,"headers":h,"security_headers":sh,"findings":findings}
...
def adaptive_site_analysis(url, scope):
    ...
    try: article_summary(result, "CyberForge Adaptive Technology Analysis")
...
def vulnerability_assessment(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"status":r.status,"security_headers":sh,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def broken_link_endpoint_audit(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"links_checked":len(rows),"results":rows,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def sensitive_object_audit(url, scope):
    ...
    for name in SENSITIVE_PATHS:
        ...
        rows.append(row); time.sleep(MIN_DELAY)
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"results":rows,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def error_misconfiguration_audit(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"status":r.status,"checks":checks,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def security_configuration_audit(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"status":r.status,"configuration":config,"findings":findings}
    score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
    return save_report("security_configuration",data)

def ssl_certificate_intelligence(url, scope):
    host=urllib.parse.urlparse(normalize_url(url)).hostname or url.split('/')[0].split(':')[0]; host=require_scope(host,scope)
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":host,"port":443}; findings=[]
    try:
        with socket.create_connection((host,443),TIMEOUT) as raw:
            with ssl.create_default_context().wrap_socket(raw,server_hostname=host) as s:
                cert=s.getpeercert(); data.update(tls_version=s.version(),cipher=s.cipher(),subject=cert.get("subject"),issuer=cert.get("issuer"),san=cert.get("subjectAltName"),not_before=cert.get("notBefore"),not_after=cert.get("notAfter"),serial=cert.get("serialNumber"),fingerprint_sha256=hashlib.sha256(s.getpeercert(binary_form=True)).hexdigest())
                if cert.get("notAfter"):
                    try:
                        exp=datetime.strptime(cert["notAfter"],"%b %d %H:%M:%S %Y %Z"); days=(exp-datetime.utcnow()).days; data["days_until_expiry"]=days
                        if days<0: findings.append(finding("high","Certificate expired",cert["notAfter"],"Renew the certificate."))
                        elif days<30: findings.append(finding("medium","Certificate expires soon",f"{days} days remaining","Plan certificate renewal."))
                    except Exception: pass
    except Exception as e: data["error"]=str(e); findings.append(finding("high","Certificate inspection failed",str(e),"Verify TLS availability and certificate configuration."))
    data["findings"]=findings; score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def robots_sitemap_audit(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"results":results,"findings":findings}; score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def redirect_security_audit(url, scope):
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"chain":chain,"final_url":current,"findings":findings}; score, level, risk_meta = risk(findings); data.update(risk_score=score,risk=level)
...
def nmap_scan(host, scope, profile="quick"):
    ...
    if not shutil.which("nmap"):
        ...
        return save_report("nmap",{"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":host,"profile":profile,"engine":"builtin-fallback","results":rows})
    profiles={
...
def ports(host,scope,plist=None):
    ...
    return save_report("ports",{"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":host,"results":rows})
...
def load_check(url,scope,count=10,delay=.5):
    ...
    return save_report("load_check",{"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,"count":count,"delay":delay,"results":rows})

def file_hash(path):
    data=hashes(path)
    print(json.dumps(data,indent=2))
    return save_report("file_hash",{"tool":"CyberForge","version":VERSION,"timestamp":stamp(),**data})
...
def _collect_findings(obj, module="General", out=None):
    ...
    if isinstance(obj, dict):
        ...
        next_module = module
        if obj.get("tool") == "CyberForge" and obj.get("target"):
            next_module = module
...
def article_summary(report, title="CyberForge Security Analysis"):
    ...
    lines.append("-----------------")
    if findings:
        lines.append(f"CyberForge-এর বিশ্লেষণে মোট {len(findings)}টি security/configuration observation পাওয়া গেছে। সামগ্রিক risk level {level}, এবং weighted risk score {score}।")
...
def article_summary_from_target(target=None):
    ...
    report={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":target or selected[-1].get("target"),"modules":selected}
...
def export_html():
    ...
    out.write_text("<!doctype html><html><meta charset='utf-8'><title>CyberForge v6.1 Report</title><style>body{font-family:monospace;background:#07120a;color:#7cff7c;padding:24px}pre{white-space:pre-wrap}</style><h1>CYBERFORGE v6.2</h1>"+''.join(sections)+"</html>",encoding="utf-8")
...
def deep_analysis(target,scope):
    ...
    adaptive=adaptive_site_analysis(url,scope)
    ...
    score, level, risk_meta = risk(findings)
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":url,
          "summary":{"finding_count":len(findings),"risk_score":score,"risk":level,"risk_analysis":risk_meta},
          "modules":{"web":web,"adaptive":adaptive,"tls":tls_result,"dns":dns_result,"geoip":geo,"nmap":nm,"ports":pr}}
    print(f"\n\033[1;32mDeep Analysis Complete\033[0m | Findings: {len(findings)} | Risk: {level}")
    save_report("deep_analysis",data)
    try: article_summary(data, "CyberForge Deep Security Analysis")
    except Exception as e: print("[!] Article summary generation failed:",e)
    return data
...
MODULE_REGISTRY = {
    ...
    30:"authentication_security_audit",31:"api_endpoint_discovery",32:"dependency_cve_intelligence",33:"http_method_security_audit",34:"risk_scoring_engine",35:"adaptive_site_analysis"
}
...
def module_health_check():
    ...
    data={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"active":active,"total":len(rows),"all_active":active==len(rows),"modules":rows}
...
def url_first_menu():
    ...
    print("\n" + "=" * 64)
    print(" CYBERFORGE — URL-FIRST ANALYSIS")
    print("=" * 64)
    ...
    while True:
        ...
        try:
            if choice == "1":
                http_posture_audit(url, scope)
            elif choice == "2":
                technology_analysis(url, scope)
            elif choice == "3":
                ssl_analysis(url, scope)
            elif choice == "4":
                dns_analysis(url, scope)
            elif choice == "5":
...
def interactive(scope_default=None):
 ...
 [38] Compliance Framework Gap Assessment
 [0]  Exit
""")
        ...
        try:
            ...
            if c=="11": malware_analyze(input(" Local file > ").strip())
                ...
                u=input(" Target URL [optional] > ").strip() or None
                f=input(" Frameworks [all/nist,iso27001,soc2,pci,gdpr,hipaa,owasp-asvs,cis] > ").strip() or "all"
                compliance_audit(u,f,scope_default,True)
...
# ---------------------------------------------------------------------------
# Compliance & Governance Framework Engine
# ---------------------------------------------------------------------------
COMPLIANCE_FRAMEWORKS = {
    "nist": {
        "name": "NIST Cybersecurity Framework 2.0",
        "controls": [
            ("GV.OC", "Organizational Context", ["scope_check", "web_audit"]),
            ("PR.AA", "Identity Management, Authentication and Access Control", ["authentication_security_audit", "cookie_security_audit"]),
            ("PR.DS", "Data Security", ["security_headers_audit", "tls", "cookie_security_audit", "cors_security_audit"]),
            ("PR.PS", "Platform Security", ["security_configuration_audit", "dependency_cve_intelligence", "technology_fingerprint"]),
            ("DE.CM", "Continuous Monitoring", ["web_audit", "cloud_exposure_audit", "waf_cdn_detection"]),
            ("DE.AE", "Adverse Event Analysis", ["error_misconfiguration_audit", "vulnerability_assessment"]),
            ("RS.MA", "Incident Management", ["vulnerability_assessment", "api_security_audit"]),
            ("RC.RP", "Incident Recovery", ["file_hash", "security_configuration_audit"]),
        ],
    },
    "iso27001": {
        "name": "ISO/IEC 27001:2022 Annex A",
        "controls": [
            ("A.5.7", "Threat intelligence", ["technology_fingerprint", "dependency_cve_intelligence"]),
            ("A.5.9", "Inventory of information and associated assets", ["technology_fingerprint", "subdomain_intelligence", "api_endpoint_discovery"]),
            ("A.5.15", "Access control", ["authentication_security_audit", "cookie_security_audit"]),
            ("A.5.23", "Information security for use of cloud services", ["cloud_exposure_audit", "waf_cdn_detection"]),
            ("A.8.8", "Management of technical vulnerabilities", ["vulnerability_assessment", "dependency_cve_intelligence"]),
            ("A.8.15", "Logging", ["error_misconfiguration_audit", "web_audit"]),
            ("A.8.20", "Networks security", ["tls", "http_method_security_audit", "cors_security_audit"]),
            ("A.8.24", "Use of cryptography", ["tls", "ssl_certificate_intelligence"]),
            ("A.8.25", "Secure development life cycle", ["javascript_security_audit", "api_security_audit"]),
            ("A.8.26", "Application security requirements", ["security_headers_audit", "web_audit"]),
        ],
    },
    "soc2": {
        "name": "SOC 2 Trust Services Criteria",
        "controls": [
            ("CC6.1", "Logical and physical access controls", ["authentication_security_audit", "cookie_security_audit"]),
            ("CC6.6", "System boundaries and restrictions", ["cors_security_audit", "http_method_security_audit", "security_configuration_audit"]),
            ("CC6.7", "Transmission of data", ["tls", "security_headers_audit"]),
            ("CC7.1", "Detection of anomalies and vulnerabilities", ["vulnerability_assessment", "dependency_cve_intelligence"]),
            ("CC7.2", "Monitoring and evaluation", ["web_audit", "cloud_exposure_audit", "waf_cdn_detection"]),
            ("CC8.1", "Change management", ["technology_fingerprint", "javascript_security_audit"]),
            ("CC9.1", "Risk mitigation", ["risk_scoring_engine", "vulnerability_assessment"]),
        ],
    },
    "pci": {
        "name": "PCI DSS v4.0.1",
        "controls": [
            ("1", "Network security controls", ["http_method_security_audit", "cors_security_audit", "waf_cdn_detection"]),
            ("2", "Secure configurations", ["security_configuration_audit", "error_misconfiguration_audit"]),
            ("4", "Protect cardholder data with strong cryptography", ["tls", "ssl_certificate_intelligence"]),
            ("6", "Develop and maintain secure systems and software", ["javascript_security_audit", "dependency_cve_intelligence", "vulnerability_assessment"]),
            ("7/8", "Restrict access and identify/authenticate users", ["authentication_security_audit", "cookie_security_audit"]),
            ("10", "Log and monitor access", ["error_misconfiguration_audit", "web_audit"]),
            ("11", "Regularly test security", ["vulnerability_assessment", "security_headers_audit"]),
        ],
    },
    "gdpr": {
        "name": "EU GDPR — Technical Security Controls",
        "controls": [
            ("Art. 25", "Data protection by design and by default", ["security_headers_audit", "cookie_security_audit", "cors_security_audit"]),
            ("Art. 32", "Security of processing", ["tls", "authentication_security_audit", "vulnerability_assessment", "security_configuration_audit"]),
            ("Art. 33", "Personal data breach detection/response support", ["error_misconfiguration_audit", "web_audit"]),
            ("Art. 35", "Data protection impact assessment support", ["technology_fingerprint", "cloud_exposure_audit", "sensitive_object_audit"]),
        ],
    },
    "hipaa": {
        "name": "HIPAA Security Rule — Technical Safeguards",
        "controls": [
            ("164.312(a)", "Access control", ["authentication_security_audit", "cookie_security_audit"]),
            ("164.312(b)", "Audit controls", ["error_misconfiguration_audit", "web_audit"]),
            ("164.312(c)", "Integrity", ["file_hash", "javascript_security_audit", "security_configuration_audit"]),
            ("164.312(d)", "Person/entity authentication", ["authentication_security_audit"]),
            ("164.312(e)", "Transmission security", ["tls", "security_headers_audit"]),
        ],
    },
    "owasp_asvs": {
        "name": "OWASP Application Security Verification Standard 5.0",
        "controls": [
            ("V1", "Architecture, design and threat modeling", ["technology_fingerprint", "api_endpoint_discovery"]),
            ("V2", "Authentication", ["authentication_security_audit", "cookie_security_audit"]),
            ("V3", "Session management", ["cookie_security_audit", "authentication_security_audit"]),
            ("V4", "Access control", ["authentication_security_audit", "cors_security_audit"]),
            ("V5", "Validation, sanitization and encoding", ["vulnerability_assessment", "api_security_audit"]),
            ("V8", "Data protection", ["tls", "security_headers_audit", "sensitive_object_audit"]),
            ("V9", "Communications", ["tls", "ssl_certificate_intelligence"]),
            ("V12", "Secure file handling", ["sensitive_object_audit", "malware_analyze"]),
            ("V13", "API and web services", ["api_security_audit", "api_endpoint_discovery", "http_method_security_audit"]),
            ("V14", "Configuration", ["security_configuration_audit", "error_misconfiguration_audit"]),
        ],
    },
    "cis": {
        "name": "CIS Critical Security Controls v8.1 — Web/External Surface",
        "controls": [
            ("CIS 1", "Asset inventory", ["subdomain_intelligence", "technology_fingerprint", "api_endpoint_discovery"]),
            ("CIS 2", "Software inventory", ["technology_fingerprint", "dependency_cve_intelligence"]),
            ("CIS 4", "Secure configuration", ["security_configuration_audit", "error_misconfiguration_audit"]),
            ("CIS 5", "Account management", ["authentication_security_audit"]),
            ("CIS 6", "Access control management", ["authentication_security_audit", "cors_security_audit"]),
            ("CIS 7", "Continuous vulnerability management", ["vulnerability_assessment", "dependency_cve_intelligence"]),
            ("CIS 13", "Network monitoring and defense", ["waf_cdn_detection", "cloud_exposure_audit"]),
            ("CIS 16", "Application software security", ["javascript_security_audit", "api_security_audit"]),
        ],
    },
}

COMPLIANCE_ALIASES = {
    "nist": "nist", "nist2": "nist", "nist-csf": "nist",
    "iso": "iso27001", "iso27001": "iso27001", "iso-27001": "iso27001",
    "soc2": "soc2", "soc-2": "soc2",
    "pci": "pci", "pci-dss": "pci",
    "gdpr": "gdpr", "hipaa": "hipaa",
    "asvs": "owasp_asvs", "owasp": "owasp_asvs", "owasp-asvs": "owasp_asvs",
    "cis": "cis", "cis-controls": "cis",
}

def _latest_report_data():
    files = sorted(REPORTS.glob("*.json"))
    for f in reversed(files):
        try:
            d = json.loads(f.read_text(encoding="utf-8"))
            if isinstance(d, dict) and (d.get("modules") or d.get("findings")):
                return d, f
        except Exception:
            continue
    return None, None

def _flatten_findings(data):
    out = []
    if not isinstance(data, dict):
        return out
    if isinstance(data.get("findings"), list):
        out.extend(x for x in data["findings"] if isinstance(x, dict))
    mods = data.get("modules", {})
    if isinstance(mods, dict):
        for value in mods.values():
            if isinstance(value, dict):
                fs = value.get("findings")
                if isinstance(fs, list):
                    out.extend(x for x in fs if isinstance(x, dict))
    # Deduplicate on stable evidence fields.
    seen, unique = set(), []
    for f in out:
        key = (
            str(f.get("severity","")).lower(),
            str(f.get("title","")).lower(),
            str(f.get("location", f.get("url",""))).lower(),
        )
        if key not in seen:
            seen.add(key); unique.append(f)
    return unique

def _finding_signal(findings, module_names):
    names = {str(x) for x in module_names}
    relevant = [f for f in findings if str(f.get("module","")) in names]
    if not relevant:
        return {"status": "NOT_ASSESSED", "score": 0, "findings": 0, "high_risk": 0}
    high = sum(1 for f in relevant if str(f.get("severity","")).lower() in {"critical","high"})
    medium = sum(1 for f in relevant if str(f.get("severity","")).lower() == "medium")
    low = sum(1 for f in relevant if str(f.get("severity","")).lower() == "low")
    # Control status is evidence-based, not a certification decision.
    if high:
        status = "GAP"
    elif medium:
        status = "PARTIAL"
        ...
        status = "OBSERVED_OK"
    score = max(0, 100 - high*30 - medium*12 - low*3)
    return {"status": status, "score": min(100, score), "findings": len(relevant), "high_risk": high}

def compliance_audit(target=None, frameworks="all", scope=None, extended=True):
    """
    Produces a technical compliance gap assessment from observable evidence.
    It does NOT certify compliance and cannot replace legal, organizational,
    physical, policy, or auditor review.
    """
    if target:
        url = normalize_url(target)
        scope = scope or temporary_scope_for_target(url)
        # Collect a broad but low-impact evidence set.
        modules = []
        runners = [
            ("web_audit", lambda: web_audit(url, scope, True)),
            ("security_headers_audit", lambda: security_headers_audit(url, scope)),
            ("tls", lambda: tls(urllib.parse.urlparse(url).hostname, scope, 443)),
            ("cookie_security_audit", lambda: cookie_security_audit(url, scope)),
            ("cors_security_audit", lambda: cors_security_audit(url, scope)),
            ("authentication_security_audit", lambda: authentication_security_audit(url, scope)),
            ("api_security_audit", lambda: api_security_audit(url, scope)),
            ("dependency_cve_intelligence", lambda: dependency_cve_intelligence(url, scope)),
            ("security_configuration_audit", lambda: security_configuration_audit(url, scope)),
            ("error_misconfiguration_audit", lambda: error_misconfiguration_audit(url, scope)),
            ("javascript_security_audit", lambda: javascript_security_audit(url, scope)),
            ("http_method_security_audit", lambda: http_method_security_audit(url, scope)),
            ("cloud_exposure_audit", lambda: cloud_exposure_audit(url, scope)),
            ("waf_cdn_detection", lambda: waf_cdn_detection(url, scope)),
            ("sensitive_object_audit", lambda: sensitive_object_audit(url, scope)),
            ("technology_fingerprint", lambda: technology_fingerprint(url, scope)),
            ("api_endpoint_discovery", lambda: api_endpoint_discovery(url, scope)),
            ("vulnerability_assessment", lambda: vulnerability_assessment(url, scope)),
        ]
        for name, runner in runners:
            if not extended and name not in {"web_audit", "security_headers_audit", "tls", "cookie_security_audit"}:
                continue
            try:
                result = runner()
                modules.append({"module": name, "result": result})
            except Exception as e:
                modules.append({"module": name, "error": str(e)})
        data = {"tool":"CyberForge", "version":VERSION, "target":url,
                "timestamp":stamp(), "modules":{m["module"]:m.get("result", {"error":m.get("error")}) for m in modules}}
    else:
        data, report_file = _latest_report_data()
        if not data:
            raise ValueError("No usable JSON report found. Run a security audit first or provide a target URL.")
        url = data.get("target")
        modules = [{"module":"existing-report","result":data}]

    raw = [x.strip().lower() for x in str(frameworks).split(",") if x.strip()]
    if not raw or "all" in raw:
        selected = list(COMPLIANCE_FRAMEWORKS)
    else:
        selected = []
        for item in raw:
            key = COMPLIANCE_ALIASES.get(item)
            if key and key not in selected:
                selected.append(key)
        if not selected:
            raise ValueError("Unknown framework. Use: all,nist,iso27001,soc2,pci,gdpr,hipaa,owasp-asvs,cis")
    ...
    for key in selected:
        ...
        controls = []
        for control_id, title, mapped_modules in fw["controls"]:
            signal = _finding_signal(findings, mapped_modules)
            controls.append({
                "control": control_id,
                "title": title,
                ...
                "related_findings": signal["findings"],
                "high_risk_findings": signal["high_risk"],
            })
        assessed = [c for c in controls if c["status"] != "NOT_ASSESSED"]
        score = round(sum(c["technical_score"] for c in assessed)/len(assessed), 1) if assessed else 0
        gaps = sum(1 for c in controls if c["status"] == "GAP")
        partial = sum(1 for c in controls if c["status"] == "PARTIAL")
        framework_results[key] = {
            "name": fw["name"],
            "technical_score": score,
            "status": "GAP" if gaps else ("PARTIAL" if partial else ("OBSERVED_OK" if assessed else "NOT_ASSESSED")),
    ...
    data_out = {
        "tool":"CyberForge", "version":VERSION, "timestamp":stamp(), "target":url,
        "assessment_type":"technical_compliance_gap_assessment",
        "disclaimer":"Technical evidence only. This report is not a legal opinion, certification, attestation, or proof of regulatory compliance.",
        "frameworks":framework_results,
        "finding_count":len(findings),
        "finding_severity_counts": {
            s: sum(1 for f in findings if str(f.get("severity","")).lower()==s)
            for s in ("critical","high","medium","low","info")
        },
    }
    save_report("compliance_audit", data_out)
    print("\n\033[1;36m=== COMPLIANCE GAP ASSESSMENT ===\033[0m")
    for key, result in framework_results.items():
        print(f" {result['name']}: {result['technical_score']}/100 | {result['status']} | gaps={result['gaps']} partial={result['partial']}")
    print("\n[!] Technical gap assessment only — not certification.")
    return data_out


def main():
    p=argparse.ArgumentParser(description="CyberForge v6.2 - authorized defensive security assessment toolkit")
    ...
    q=sub.add_parser("compliance-audit"); q.add_argument("url", nargs="?"); q.add_argument("--frameworks", default="all"); q.add_argument("--no-extended", action="store_true")
    ...
    try:
        ...
        elif a.cmd=="compliance-audit": compliance_audit(a.url,a.frameworks,a.scope,not a.no_extended)
