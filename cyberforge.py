...
# --- CyberForge Compliance Framework Engine ---
COMPLIANCE_FRAMEWORKS = {
 "nist":("NIST CSF 2.0",["web_audit","security_headers_audit","tls","cookie_security_audit","cors_security_audit","security_configuration_audit","dependency_cve_intelligence","technology_fingerprint","cloud_exposure_audit","waf_cdn_detection","vulnerability_assessment","api_security_audit"]),
 "iso27001":("ISO/IEC 27001:2022 Annex A",["authentication_security_audit","cookie_security_audit","cloud_exposure_audit","vulnerability_assessment","dependency_cve_intelligence","error_misconfiguration_audit","tls","javascript_security_audit","api_security_audit","security_headers_audit"]),
 "soc2":("SOC 2 Trust Services Criteria",["authentication_security_audit","cookie_security_audit","cors_security_audit","tls","security_headers_audit","vulnerability_assessment","web_audit","cloud_exposure_audit"]),
 "pci":("PCI DSS 4.0.1",["http_method_security_audit","cors_security_audit","waf_cdn_detection","security_configuration_audit","tls","authentication_security_audit","vulnerability_assessment","dependency_cve_intelligence"]),
 "gdpr":("EU GDPR — Technical Security Controls",["security_headers_audit","cookie_security_audit","cors_security_audit","tls","authentication_security_audit","vulnerability_assessment","security_configuration_audit","cloud_exposure_audit"]),
 "hipaa":("HIPAA Security Rule — Technical Safeguards",["authentication_security_audit","cookie_security_audit","error_misconfiguration_audit","web_audit","tls","security_headers_audit"]),
 "owasp_asvs":("OWASP ASVS 5.0",["technology_fingerprint","api_endpoint_discovery","authentication_security_audit","cookie_security_audit","cors_security_audit","vulnerability_assessment","api_security_audit","tls","sensitive_object_audit","security_configuration_audit"]),
 "cis":("CIS Critical Security Controls v8.1",["subdomain_intelligence","technology_fingerprint","api_endpoint_discovery","security_configuration_audit","authentication_security_audit","vulnerability_assessment","dependency_cve_intelligence","waf_cdn_detection","cloud_exposure_audit","javascript_security_audit"])
}
COMPLIANCE_ALIASES={"nist":"nist","iso":"iso27001","iso27001":"iso27001","soc2":"soc2","pci":"pci","pci-dss":"pci","gdpr":"gdpr","hipaa":"hipaa","asvs":"owasp_asvs","owasp":"owasp_asvs","cis":"cis"}

def _cf_findings(data):
    out=[]
    if not isinstance(data,dict): return out
    if isinstance(data.get("findings"),list): out += [x for x in data["findings"] if isinstance(x,dict)]
    for v in data.get("modules",{}).values() if isinstance(data.get("modules"),dict) else []:
        if isinstance(v,dict) and isinstance(v.get("findings"),list): out += [x for x in v["findings"] if isinstance(x,dict)]
    return out

def compliance_audit(target=None,frameworks="all",scope=None,extended=True):
    if target:
        url=normalize_url(target); scope=scope or temporary_scope_for_target(url)
        runners=[
         ("web_audit",lambda:web_audit(url,scope,True)),("security_headers_audit",lambda:security_headers_audit(url,scope)),
         ("tls",lambda:tls(urllib.parse.urlparse(url).hostname,scope,443)),("cookie_security_audit",lambda:cookie_security_audit(url,scope)),
         ("cors_security_audit",lambda:cors_security_audit(url,scope)),("authentication_security_audit",lambda:authentication_security_audit(url,scope)),
         ("api_security_audit",lambda:api_security_audit(url,scope)),("dependency_cve_intelligence",lambda:dependency_cve_intelligence(url,scope)),
         ("security_configuration_audit",lambda:security_configuration_audit(url,scope)),("error_misconfiguration_audit",lambda:error_misconfiguration_audit(url,scope)),
         ("javascript_security_audit",lambda:javascript_security_audit(url,scope)),("http_method_security_audit",lambda:http_method_security_audit(url,scope)),
         ("cloud_exposure_audit",lambda:cloud_exposure_audit(url,scope)),("waf_cdn_detection",lambda:waf_cdn_detection(url,scope)),
         ("sensitive_object_audit",lambda:sensitive_object_audit(url,scope)),("technology_fingerprint",lambda:technology_fingerprint(url,scope)),
         ("api_endpoint_discovery",lambda:api_endpoint_discovery(url,scope)),("vulnerability_assessment",lambda:vulnerability_assessment(url,scope))]
        modules={}
        for n,fn in runners:
            if not extended and n not in {"web_audit","security_headers_audit","tls","cookie_security_audit"}: continue
            try: modules[n]=fn()
            except Exception as e: modules[n]={"error":str(e)}
        data={"tool":"CyberForge","version":VERSION,"target":url,"timestamp":stamp(),"modules":modules}
    else:
        files=sorted(REPORTS.glob("*.json")); data=None
        for f in reversed(files):
            try:
                x=json.loads(f.read_text(encoding="utf-8"))
                if isinstance(x,dict): data=x; break
            except Exception: pass
        if not data: raise ValueError("No JSON report found.")
    keys=[x.strip().lower() for x in str(frameworks).split(",") if x.strip()]
    selected=list(COMPLIANCE_FRAMEWORKS) if not keys or "all" in keys else [COMPLIANCE_ALIASES[x] for x in keys if x in COMPLIANCE_ALIASES]
    if not selected: raise ValueError("Unknown framework.")
    findings=_cf_findings(data); results={}
    for k in selected:
        name,mapped=COMPLIANCE_FRAMEWORKS[k]
        rel=[f for f in findings if str(f.get("module","")) in mapped]
        high=sum(str(f.get("severity","")).lower() in {"critical","high"} for f in rel)
        med=sum(str(f.get("severity","")).lower()=="medium" for f in rel)
        results[k]={"name":name,"technical_score":max(0,100-high*25-med*10),"status":"GAP" if high else ("PARTIAL" if med else ("OBSERVED_OK" if rel else "NOT_ASSESSED")),"related_findings":len(rel),"high_risk_findings":high,"mapped_modules":mapped}
    out={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"target":data.get("target"),"assessment_type":"technical_compliance_gap_assessment","disclaimer":"Technical evidence only; not certification or legal advice.","frameworks":results}
    save_report("compliance_audit",out)
    return out

# --- Authorized Activity & Log Audit / SIEM-lite ---
def _load_activity_events(path):
    text=Path(path).read_text(encoding="utf-8",errors="replace")
    try:
        x=json.loads(text)
        if isinstance(x,list): return [e for e in x if isinstance(e,dict)]
        if isinstance(x,dict) and isinstance(x.get("events"),list): return [e for e in x["events"] if isinstance(e,dict)]
    except Exception: pass
    events=[]
...
def activity_log_audit(log_path,output_name="activity_audit"):
    events=_load_activity_events(log_path); counts={}; ips={}; users={}; suspicious=[]
    for e in events:
        typ=str(e.get("event",e.get("event_type",e.get("action","")))).lower().replace(" ","_")
        counts[typ]=counts.get(typ,0)+1
        ip=str(e.get("ip",e.get("source_ip","unknown"))); ips[ip]=ips.get(ip,0)+1
        user=str(e.get("user",e.get("username",e.get("email","unknown")))); users[user]=users.get(user,0)+1
        if typ in {"privilege_change","admin_action","api_key_change"}:
            suspicious.append({"type":"sensitive_action","user":user,"ip":ip,"event":typ})
    failed={}
    for e in events:
        typ=str(e.get("event",e.get("event_type",e.get("action","")))).lower().replace(" ","_")
        if typ in {"login_failure","failed_login","failure","failed"}:
            ip=str(e.get("ip",e.get("source_ip","unknown"))); failed[ip]=failed.get(ip,0)+1
    for ip,n in failed.items():
        if n>=5: suspicious.append({"type":"authentication_anomaly","ip":ip,"failed_attempts":n})
    out={"tool":"CyberForge","version":VERSION,"timestamp":stamp(),"assessment_type":"authorized_activity_log_audit","source":str(log_path),"event_count":len(events),"event_counts":counts,"top_ips":sorted(ips.items(),key=lambda x:x[1],reverse=True)[:20],"top_users":sorted(users.items(),key=lambda x:x[1],reverse=True)[:20],"suspicious_activity":suspicious,"risk":"HIGH" if suspicious else "LOW","disclaimer":"Use only logs/telemetry you own or are authorized to access."}
    save_report(output_name,out); return out
...
def main():
    ...
    q=sub.add_parser("activity-audit"); q.add_argument("logfile"); q.add_argument("--output",default="activity_audit")
    ...
    try:
        ...
        elif a.cmd=="activity-audit": activity_log_audit(a.logfile,a.output)
